...
{	
	...
	if(rxSensor>sxSensor && cSensor < 70 && (cs.getAngle()<0.001 || cs.getTrackPos()<0) ) 
	 ...
	 if (cs.getTrackPos() < 1 && cs.getTrackPos() > -1)
    {
		...
		else
        {
            ...
            if(rxSensor>sxSensor)
            {
				...
				if(cSensor<50 && rxSensor-sxSensor<10)
					return 0.6* maxSpeed*(cSensor*sqrt(sinAngle)/maxSpeedDist);
