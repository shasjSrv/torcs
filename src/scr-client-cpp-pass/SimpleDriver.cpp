...
const float SimpleDriver::maxSpeed=230;
    ...
    if(cs.getTrackPos()<=1&&cs.getTrackPos()>=-1)
    {
        if(side_pid==NULL)
        {
            ...
            if(targetSide==1)
            {
                ...
                //if(cs.getOpponents(16)<oppo) oppo=cs.getOpponents(16);
                //if(cs.getOpponents(20)<oppo) oppo=cs.getOpponents(20);
                //20m以内可判定一定在同车道

                if(oppo<0.3*cs.getSpeedX()/3.6){
                    targetSide=-1;
                    side_pid->setP(0.6);
                    side_pid->setI(0.0002);
                    side_pid->setD(0.4);
                    cout<<"change.."<<endl;
                }
            }
            ...
            if(targetSide==1){
                ...
                targetAngle=side_pid->refresh(cs.getTrackPos()-0.2,0.02);  //正的要向右转
            }
    ...
    if (cs.getTrackPos() < 1 && cs.getTrackPos() > -1)
    {
        ...
        else
        {
            ...
            if(rxSensor>sxSensor)
            {
                ...
                if(normal_targetSpeed<80) normal_targetSpeed=80;
            }
